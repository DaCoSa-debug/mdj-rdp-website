...
export default function BattleshipLobby() {
  ...
  if (room) {
    ...
    return <main className="min-h-screen bg-[#231F20] px-4 py-7 text-white sm:px-6"><div className="mx-auto w-full max-w-2xl"><Link to="/arcade" onClick={leaveRoom} className="text-sm font-bold text-white/65 hover:text-white">← Retour à l’arcade</Link><section className="mt-6 rounded-[2rem] border border-[#29ABE2]/25 bg-[#2a2020] p-5 shadow-2xl sm:p-8"><p className="text-center text-xs font-black tracking-[.22em] text-[#F05063]">BATAILLE NAVALE · MDJ</p><h1 className="mt-2 text-center text-3xl font-black sm:text-4xl">Salle {room.code}</h1><div className={`mt-5 rounded-2xl px-4 py-3 text-center font-black ${full ? 'bg-[#29ABE2]/15 text-[#9edfff]' : 'bg-white/5 text-white/75'}`}>{full ? '2/2 joueurs connectés' : `${room.players.length}/2 joueurs connectés`}</div>{!full && <div className="mt-6 rounded-2xl bg-white p-4 text-center text-[#231F20]"><p className="mb-3 text-sm font-bold">Invite un ami avec ce QR ou le code.</p><div className="mx-auto w-fit rounded-xl bg-white p-2"><QRCodeSVG value={inviteUrl} size={180} level="M" includeMargin /></div><p className="mt-3 font-mono text-2xl font-black tracking-[.28em]">{room.code}</p></div>}{full && (!battle || !battle.yourFleetReady) && <div className="mt-6 text-center"><div className="rounded-2xl bg-[#29ABE2]/10 p-4 text-left text-sm text-white/75"><p className="font-black text-[#9edfff]">COMMENT JOUER</p><p className="mt-1">1. Place ta flotte. 2. Attends ton tour. 3. Tape une case rouge pour tirer.</p></div><button onClick={readyForBattle} className="mt-5 min-h-14 rounded-2xl bg-gradient-to-r from-[#29ABE2] to-[#F05063] px-7 font-black active:scale-[.98]">PLACER MES NAVIRES</button></div>}{battle && <div className="mt-6"><p className={`rounded-xl px-3 py-2 text-center text-sm font-black ${battle.phase === 'finished' ? 'bg-[#F05063]/20 text-[#ffb1ba]' : battle.yourTurn ? 'bg-[#29ABE2]/20 text-[#9edfff]' : 'bg-white/5 text-white/55'}`}>{battle.phase === 'placement' ? 'Flotte prête — attends ton adversaire…' : battle.phase === 'finished' ? (won ? '🏆 Tu as gagné !' : '💥 Ton adversaire gagne cette manche.') : battle.yourTurn ? 'À TOI DE JOUER' : 'Tour de ton adversaire…'}</p>{effect && <p className="mt-3 animate-pulse text-center text-xl font-black text-[#FBB040]">{effect}</p>}{emote && <p className="mt-3 animate-bounce text-center text-2xl font-black text-white">{emote}</p>}<div className="mt-4 flex justify-center gap-2">{EMOTES.map(emoji => <button key={emoji} onClick={() => sendEmote(emoji)} className="min-h-11 min-w-11 rounded-xl bg-white/10 text-xl active:scale-90">{emoji}</button>)}</div><div className="mt-5 grid gap-5 md:grid-cols-2"><div><p className="mb-2 text-xs font-black tracking-widest text-[#29ABE2]">{me?.nickname ?? 'TOI'} · TES NAVIRES</p><BattleBoard cells={battle.yourBoard} /></div><div><p className="mb-2 text-xs font-black tracking-widest text-[#F05063]">{opponent?.nickname ?? 'ADVERSAIRE'} · À VISER</p><BattleBoard cells={battle.targetBoard} target active={battle.phase === 'playing' && battle.yourTurn} onFire={fire} /></div></div><p className="mt-4 text-center text-xs text-white/45">Bleu: navires · Rouge: touché · Gris: à l’eau</p></div>}<button onClick={leaveRoom} className="mt-6 min-h-12 w-full rounded-2xl border border-white/20 font-black text-white/75 hover:bg-white/10">Quitter la salle</button></section></div></main>
  }
