...
export default function BattleshipLobby() {
  ...
  const [celebration, setCelebration] = useState<'sunk' | 'win' | null>(null)
  ...
  useEffect(() => {
    ...
    socket.on('game:effect', payload => {
      setEffect(payload.type === 'sunk' ? '🚢 NAVIRE COULÉ !' : payload.type === 'hit' ? '💥 TOUCHÉ !' : payload.type === 'miss' ? '🌊 À L’EAU !' : payload.type === 'win' ? '🏆 VICTOIRE !' : 'À TOI !')
      if (payload.type === 'sunk' || payload.type === 'win') {
        setCelebration(payload.type)
        if (payload.type === 'sunk') window.setTimeout(() => setCelebration(null), 1800)
      }
      window.setTimeout(() => setEffect(''), 1500)
    })
